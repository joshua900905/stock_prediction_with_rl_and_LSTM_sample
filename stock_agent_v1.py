...
# Version: 1.9.2 (Target Trade Value)
...
try:
    ...
    from price_prediction_v1 import SCALER_FILENAME as SCALER_FILENAME_TPL # Import template
...
def get_ohlc_data_for_stock(stock_code):
    ...
    if not ohlc_data: print(f"ERROR (get_ohlc {stock_code}): No valid records."); return None
...
def getState_OHLC_Holding(ohlc_data,t_index,window_size,scaler,holding_status):
    ...
    if end>len(ohlc_data): print("ERROR (getState): Index out of bounds."); return fallback
    window_data=ohlc_data[start:end,:];
    if window_data.shape!=(window_size,n_feat): print("ERROR (getState): Window shape mismatch."); return fallback
...
class Agent:
    def __init__(self, window_size, is_eval=False, model_path=""): # ... (same as V1.9.1) ...
    ...
    def _load_or_build_model(self, model_path): # ... (same validation) ...
    ...
    def _build_model(self): # ... (same structure) ...
    ...
    def remember(self, state, action, reward, next_state, done): # ... (same logic) ...
         if not self.is_eval:
    ...
    def replay(self, batch_size): # ... (same logic) ...
         ...
         if states.shape[1] != self.state_size or next_states.shape[1] != self.state_size: print(f"ERROR (replay): Batch state dim mismatch."); return
         try:
    ...
    def save(self, name): # ... (same logic) ...
        if hasattr(self, 'model') and self.model: 
            try: self.model.save(name); print(f"RL model saved: {name}.") 
            except Exception as e: print(f"ERROR saving RL model: {e}")
        else: print("ERROR (save): No model.")


# === RL Training Function (Refactored Placeholder - Needs Correct Implementation) ===
...
def trading_execution():
    ...
    # --- Config & Setup ---
    # shares_to_trade calculated dynamically
    ...
    team_account_history=TEAM_ACCOUNT_ID; day_trade_sell_method=DAY_TRADE_SELL_METHOD
    ...
    for stock_code in target_stock_codes:
        ...
        # ... (Reconciliation logic - same as V1.7/1.9.1) ...
        ...
        final_action_stock = 0 # Default to Sit if state fails
        action_reason = "State Error (Default Sit)"
        ...
        else: day_trade_sell_price = predicted_close if predicted_close is not None else buy_order_price # Fallback
        ...
        # Calculate shares
        shares_to_trade = MIN_SHARES_PER_TRADE
        shares_to_sell_calculated = MIN_SHARES_PER_TRADE # Default for selling
        if predicted_open > 0:
             calculated_shares = int(TARGET_TRADE_VALUE / (predicted_open * 1000.0))
             shares_to_trade = max(MIN_SHARES_PER_TRADE, calculated_shares)
             shares_to_sell_calculated = min(actual_shares_held, shares_to_trade) # Max sellable is min(held, target)
             print(f"      Calc Shares: TargetVal {TARGET_TRADE_VALUE:,.0f} / ({predicted_open:.2f}*1k) => {calculated_shares} lots. Trade: {shares_to_trade} lots. Max Sell: {shares_to_sell_calculated} lots.")
        ...
        if final_action_stock == 1: # Buy
            if not final_confirmed_holding:
                print(f"      ACTION: Placing BUY {shares_to_trade} shares @ {buy_order_price:.2f}")
                if Buy_Stock(user_account_stock, user_password_stock, stock_code, shares_to_trade, buy_order_price):
                     ...
                     print(f"      ACTION: Placing DayTrade SELL {shares_to_trade} shares @ {day_trade_sell_price:.2f}")
            ...
            if final_confirmed_holding:
                 if shares_to_sell_calculated >= MIN_SHARES_PER_TRADE: # Use calculated sellable shares
                     print(f"      ACTION: Placing SELL {shares_to_sell_calculated} shares @ {sell_order_price:.2f} (Reason: {action_reason})")
                 ...
                 else: print(f"      INFO: SELL action, but sellable shares ({shares_to_sell_calculated}) < min ({MIN_SHARES_PER_TRADE}). No action.")
    ...
    save_trade_info(trade_info) # Save the updated dictionary
...
if __name__ == "__main__":
    print("="*40); print("   股票交易機器人 (v1.9.2 - 固定列表/目標金額)"); print("="*40)
    ...
    #    Training part needs separate, correct implementation for multi-stock and delay.
